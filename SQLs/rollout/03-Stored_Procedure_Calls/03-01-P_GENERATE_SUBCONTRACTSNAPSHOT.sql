create or replace
Procedure                   P_GENERATE_SUBCONTRACTSNAPSHOT Is
  ...
  SCPACKAGE_SNAPSHOT PCMSDATADEV.SUBCONTRACT_SNAPSHOT%ROWTYPE;
  SCPACKAGE PCMSDATADEV.SUBCONTRACT%ROWTYPE;
  
  TYPE Rcursor IS REF CURSOR;
  SNAPSHOTCURSOR Rcursor;
  
  CURSOR PACKAGECURSOR IS 
                      SELECT * FROM PCMSDATADEV.SUBCONTRACT WHERE SYSTEM_STATUS = 'ACTIVE';
...
BEGIN    
    ...
    OPEN SNAPSHOTCURSOR FOR
          SELECT * FROM PCMSDATADEV.SUBCONTRACT_SNAPSHOT WHERE SYSTEM_STATUS = 'ACTIVE' AND SNAPSHOT_DATE >= FROMDATE AND SNAPSHOT_DATE <= THRUDATE;
    LOOP
        FETCH SNAPSHOTCURSOR INTO SCPACKAGE_SNAPSHOT;
        EXIT WHEN SNAPSHOTCURSOR%NOTFOUND;
        
        UPDATE PCMSDATADEV.SUBCONTRACT_SNAPSHOT
    ...
    SELECT MAX(ID) INTO PID FROM PCMSDATADEV.SUBCONTRACT_SNAPSHOT;
     IF (PID is null) THEN
        PID := 0;
    END IF;
    ...
    LOOP
        ...
        INSERT INTO PCMSDATADEV.SUBCONTRACT_SNAPSHOT(ID, LAST_MODIFIED_DATE, LAST_MODIFIED_USER, CREATED_DATE, CREATED_USER, SYSTEM_STATUS, SUBCONTRACT_ID, JOB_INFO_ID, PACKAGENO, 
                    ...
                    SNAPSHOT_DATE, NAME_SUBCONTRACTOR, NOTES, WORK_SCOPE, AMOUNT_TOTAL_AP_POSTED_CERT)
        VALUES (PID, SYSDATE, 'SYSTEM', SYSDATE, 'SYSTEM', 'ACTIVE', SCPACKAGE.ID, SCPACKAGE.JOB_INFO_ID, SCPACKAGE.PACKAGENO, SCPACKAGE.DESCRIPTION, SCPACKAGE.PACKAGETYPE, SCPACKAGE.VENDORNO, 
                ...
                SNAPSHOTDATE, SCPACKAGE.NAME_SUBCONTRACTOR, SCPACKAGE.NOTES, SCPACKAGE.WORK_SCOPE, SCPACKAGE.AMOUNT_TOTAL_AP_POSTED_CERT);
