create or replace Procedure PCMSDATATST.P_GENERATE_SUBCONTRACTSNAPSHOT Is
  ...
  SCPACKAGE_SNAPSHOT PCMSDATATST.SUBCONTRACT_SNAPSHOT%ROWTYPE;
  SCPACKAGE PCMSDATATST.SUBCONTRACT%ROWTYPE;
  
  TYPE Rcursor IS REF CURSOR;
  SNAPSHOTCURSOR Rcursor;
  
  CURSOR PACKAGECURSOR IS 
                      SELECT * FROM PCMSDATATST.SUBCONTRACT WHERE SYSTEM_STATUS = 'ACTIVE';
...
BEGIN    
    ...
    OPEN SNAPSHOTCURSOR FOR
          SELECT * FROM PCMSDATATST.SUBCONTRACT_SNAPSHOT WHERE SYSTEM_STATUS = 'ACTIVE' AND SNAPSHOT_DATE >= FROMDATE AND SNAPSHOT_DATE <= THRUDATE;
    LOOP
        FETCH SNAPSHOTCURSOR INTO SCPACKAGE_SNAPSHOT;
        EXIT WHEN SNAPSHOTCURSOR%NOTFOUND;
        
        UPDATE PCMSDATATST.SUBCONTRACT_SNAPSHOT
    ...
    SELECT MAX(ID) INTO PID FROM PCMSDATATST.SUBCONTRACT_SNAPSHOT;
     IF (PID is null) THEN
        PID := 0;
    END IF;
    ...
    LOOP
        ...
        INSERT INTO PCMSDATATST.SUBCONTRACT_SNAPSHOT(ID, LAST_MODIFIED_DATE, LAST_MODIFIED_USER, CREATED_DATE, CREATED_USER, SYSTEM_STATUS, SUBCONTRACT_ID, JOB_INFO_ID, PACKAGENO, 
