...
UPDATE PCMSDATAPROD.SUBCONTRACT_DETAIL set AMT_SUBCONTRACT = round(round(QUANTITY, 4) * round(SCRATE, 4), 2) where SCRATE is not null and APPROVED = 'A';
UPDATE PCMSDATAPROD.SUBCONTRACT_DETAIL set AMT_SUBCONTRACT = round(round(TOBEAPPROVEDQTY, 4) * round(SCRATE, 4), 2) where SCRATE is not null and APPROVED != 'A';

UPDATE PCMSDATAPROD.SUBCONTRACT_DETAIL set AMT_SUBCONTRACT_NEW = AMT_SUBCONTRACT where SCRATE is not null;
UPDATE PCMSDATAPROD.SUBCONTRACT_DETAIL set AMT_BUDGET = round(round(QUANTITY, 4) * round(COSTRATE, 4), 2) where COSTRATE is not null and APPROVED = 'A';
...
--------------------------------------------------------
 -- select * from PCMSDATAPROD.RESOURCE_SUMMARY;
UPDATE PCMSDATAPROD.RESOURCE_SUMMARY set AMT_BUDGET = round(round(QUANTITY, 4) * round(RATE, 4), 2) where RATE is not null;

--------------------------------------------------------
--  Calculate BPI Item Resource new columns
--------------------------------------------------------
-- select * from PCMSDATAPROD.BPI_ITEM_RESOURCE;
UPDATE PCMSDATAPROD.BPI_ITEM_RESOURCE set AMT_BUDGET = round(round(QUANTITY, 4) * round(COSTRATE, 4) * round(REMEASUREDFACTOR, 4), 2) where COSTRATE is not null;

--------------------------------------------------------
--  Calculate BPI Item new columns
--------------------------------------------------------
-- select * from PCMSDATAPROD.BPI_ITEM;
UPDATE PCMSDATAPROD.BPI_ITEM set AMT_BUDGET = round(round(REMEASUREDQTY, 4) * round(COSTRATE, 4), 2) where COSTRATE is not null;
UPDATE PCMSDATAPROD.BPI_ITEM set AMT_SELLING = round(round(REMEASUREDQTY, 4) * round(SELLINGRATE, 4), 2) where SELLINGRATE is not null and REMEASUREDQTY * SELLINGRATE > -99999999999999999 and  REMEASUREDQTY * SELLINGRATE < 99999999999999999;
