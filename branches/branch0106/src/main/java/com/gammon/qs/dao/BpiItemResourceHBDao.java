...
public class BpiItemResourceHBDao extends BaseHibernateDao<BpiItemResource> {
	...
	public boolean saveResource(BpiItemResource resource) throws DatabaseOperationException{
		if (resource==null)
	...
	public boolean updateResource(BpiItemResource resource) throws DatabaseOperationException{
		if (resource==null)
			throw new NullPointerException("Resource is null");
		try {
			BpiItemResource dbResource = getResource(resource);
			if (dbResource!=null){
...
//			logger.info("DONE -> getTotalResourceAmountbyBPI() - Total Resource Amount: "+totalResourceAmount);
	 ...
	 * @throws DatabaseOperationException 
	 */
	public Boolean updateResourcesAfteringPosting(JobInfo jobInfo, String username) throws DatabaseOperationException {
	 ...
	 * @throws DatabaseOperationException 
	 */
	public BpiItemResource obtainResource(	String jobNumber, String billNo, String subBillNo, String pageNo, String itemNo, String packageNo,
...
//				" Resource Rate:"+resourceRate+" Unit:"+unit);
		BpiItemResource result = new BpiItemResource();
		...
		try{
			...
			result = (BpiItemResource)criteria.uniqueResult();;
		}catch(HibernateException ex){
			throw new DatabaseOperationException(ex);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IVBpiItemGroupedByIDWrapper> groupResourcesToBpiItems(String jobNumber) throws DatabaseOperationException {
		logger.info("GROUP RESOURCE TO BQITEM BY: BQItem ID for J#"+jobNumber);
		
		Criteria criteria = getSession().createCriteria(this.getType());
		List<IVBpiItemGroupedByIDWrapper> result = new ArrayList<IVBpiItemGroupedByIDWrapper>();
		...
		try{
			...
			criteria.setProjection(Projections.projectionList()
					...
					.add(Projections.groupProperty("bpiItem.id"), "bpiItemID"));
			
			criteria.setResultTransformer(new AliasToBeanResultTransformer(IVBpiItemGroupedByIDWrapper.class));
			result = (List<IVBpiItemGroupedByIDWrapper>)criteria.list();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IVBpiResourceSummaryGroupedBySCWrapper> groupResourcesToBQResourceSummariesForSC(String jobNumber) throws DatabaseOperationException, ValidateBusinessLogicException {
		logger.info("GROUP RESOURCE TO BQRESOURCESUMMARY BY: Subcontract for J#"+jobNumber);
		Criteria criteria = getSession().createCriteria(this.getType());
		List<IVBpiResourceSummaryGroupedBySCWrapper> result = new ArrayList<IVBpiResourceSummaryGroupedBySCWrapper>();
		...
		try{
			...
			criteria.setResultTransformer(new AliasToBeanResultTransformer(IVBpiResourceSummaryGroupedBySCWrapper.class));
			result = (List<IVBpiResourceSummaryGroupedBySCWrapper>)criteria.list();
			
			logger.info("NUMBER OF RECORDS After grouping: "+result.size());
		}catch(HibernateException e){
			logger.info("Fail: groupResourcesToWrappersBySC(JobInfo jobInfo)");
			throw new DatabaseOperationException(e);
		}
		return result;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IVBpiResourceSummaryGroupedBySCWrapper> groupResourcesToBpiResourceSummariesForNonSC(String jobNumber) throws DatabaseOperationException, ValidateBusinessLogicException {
		...
		try{		
			...
			logger.info("Fail: groupResourcesToBQResourceSummariesForNonSC(JobInfo jobInfo)");
			throw new DatabaseOperationException(e);
		}
		return result;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<BpiItemResource> obtainSCResources(String jobNumber) throws DatabaseOperationException {
		Criteria criteria = getSession().createCriteria(this.getType());
		List<BpiItemResource> result = new ArrayList<BpiItemResource>();
		try{
			//where
			criteria.add(Restrictions.eq(SYSTEM_STATUS, BasePersistedAuditObject.ACTIVE));
			criteria.add(Restrictions.eq(JOB_NUMBER, jobNumber));
			...
			result = (List<BpiItemResource>) criteria.list();
	 ...
	 * @throws DatabaseOperationException 
	 */
	public boolean updateResourceIVAmountByHQL(BpiItemResource resource, String username) throws DatabaseOperationException {
//		logger.info("UPDATE: updateResourceIVAmountByHQL(Resource resource, String username)");
		try{
			...
			String hql = "UPDATE BpiItemResource SET ivCumAmount = :updatedIVCumulativeAmount, ivCumQuantity = :updatedIVCumulativeQuantity, ivMovementAmount = :updatedIVMovementAmount, lastModifiedUser = :username, lastModifiedDate = :date WHERE id = :resourceID AND jobNumber = :jobNumber";
			Query query = getSession().createQuery(hql);
