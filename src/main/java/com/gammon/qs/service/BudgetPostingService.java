...
//		adminService.canAccessJob(jobNumber);
		...
		try {
			job = jobWsDao.obtainJob(jobNumber);
			...
			postingTime = 10000 * cal.get(Calendar.HOUR_OF_DAY) + 100 * cal.get(Calendar.MINUTE)
			...
			for (BudgetPostingWrapper accountBudget : accountBudgets) {
				...
				ValidateAccNumResponseObj accNumResponse = budgetPostingDao.getAccNum(jobNumber, objectCode,
						subsidiaryCode);
				if (accNumResponse.getAccountID() == null || accNumResponse.getAccountID().trim().length() == 0) {
					logger.info("Account code not in jde master list. Job: " + job.getJobNumber() + ", Object: "
							+ accountBudget.getObjectCode() + ", Subsid: " + accountBudget.getSubsidiaryCode());
					logger.info("Try to create Account");
					String errMsg = masterListRepository.validateAndCreateAccountCode(jobNumber, objectCode,
							subsidiaryCode);
					if (errMsg != null) {
			...
			ValidateAAICompletelyResponseObj aaiResponse = budgetPostingDao.getProjectMarginAai(jobNumber,
					job.getCompany());
			if (aaiResponse == null)
				return "An error occurred when trying to validate an account";
			else if (aaiResponse.getAccountNumberInputModeUnknown() == null
					|| aaiResponse.getAccountNumberInputModeUnknown().length() == 0) {
