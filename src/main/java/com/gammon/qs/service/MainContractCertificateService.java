...
import com.gammon.qs.dao.RetentionReleaseScheduleHBDao;
...
public class MainContractCertificateService {
	/**
	 * @author xethhung
	...
	private List<MainContractCertificate> cachedMainContractCert = new ArrayList<MainContractCertificate>();
	...
	@Autowired
	private RetentionReleaseScheduleHBDao retentionReleaseScheduleHBDaoImpl;
	...
	public String updateMainContractCert(MainContractCertificate updatedMainCert) throws DatabaseOperationException {
		...
		if (updatedMainCert!=null){
			...
			MainContractCertificate currMainCert = mainContractCertificateHBDaoImpl.obtainMainContractCert(updatedMainCert.getJobNo(), updatedMainCert.getCertificateNumber());
	...
	private void setModifiedDate(MainContractCertificate mainContractCertificate){
	...
	******************************************/
	...
	public String addMainCertContraCharge(MainCertificateContraCharge mainCertContraCharge)throws DatabaseOperationException{
		...
		MainContractCertificate mainCert = new MainContractCertificate();
	...
	public boolean deleteMainCertContraCharge(MainCertificateContraCharge mainCertContraCharge) throws DatabaseOperationException{
		MainCertificateContraCharge mainCertCC = mainCertContraChargeHBDaoImpl.obtainMainCertContraCharge(mainCertContraCharge.getObjectCode(), mainCertContraCharge.getSubsidiary(), mainCertContraCharge.getMainCertificate());
	...
	public void setTotalContraChargeAmt(MainContractCertificate mainCert, List<MainCertificateContraCharge> mainCertificateContraChargeList) throws DatabaseOperationException{
		Double totalCCAmt = 0.00;
		for(MainCertificateContraCharge curCC : mainCertificateContraChargeList)
	...
	public String insertAndPostMainContractCert(String jobNumber, Integer mainCertNumber, Date asAtDate) throws DatabaseOperationException {
		...
		MainContractCertificate mainCert = getMainContractCert(jobNumber, mainCertNumber);
		List<MainCertificateContraCharge> mainCertContraChargeList = getMainCertCertContraChargeList(mainCert);
		...
		for(MainCertificateContraCharge mainCertContraCharge:mainCertContraChargeList)
		...
		MainContractCertificate toBeUpdatedMainCert = getMainContractCert(mainCertWrapper.getJobNumber(), mainCertWrapper.getCertificateNumber());
		toBeUpdatedMainCert.setCertificateStatus(MainContractCertificate.CERT_POSTED);
	...
	public String insertIPAAndUpdateMainContractCert(MainContractCertificate mainCert) throws DatabaseOperationException{
		...
		MainContractCertificate toBeUpdatedMainCert = getMainContractCert(mainCert.getJobNo(), mainCert.getCertificateNumber());
	...
	public String resetMainContractCert(MainContractCertificate toBeUpdatedMainCert) throws DatabaseOperationException{
		...
		if(toBeUpdatedMainCert!=null){		
			...
			MainContractCertificate mainCertInDB = mainContractCertificateHBDaoImpl.obtainMainContractCert(toBeUpdatedMainCert.getJobNo(), toBeUpdatedMainCert.getCertificateNumber());
			
			if(mainCertInDB!=null){
				if(mainCertInDB.getCertificateStatus()!=null && mainCertInDB.getCertificateStatus().equals(MainContractCertificate.CERT_CONFIRMED)){
					mainCertInDB.setCertificateStatus(MainContractCertificate.IPA_SENT);
	...
	public String confirmMainContractCert(MainContractCertificate toBeUpdatedMainCert) throws DatabaseOperationException {
		...
		if(toBeUpdatedMainCert!=null){		
			...
			if(mainCertInDB!=null){
				if(mainCertInDB.getCertificateStatus()==null || !mainCertInDB.getCertificateStatus().equals(MainContractCertificate.IPA_SENT)){
					message = 	"Main Contract Certificate Job: "+toBeUpdatedMainCert.getJobNo()+" Certificate No.: "+toBeUpdatedMainCert.getCertificateNumber()+" \n"+ 
					...
					mainCertInDB.setCertificateStatus(MainContractCertificate.CERT_CONFIRMED);
					mainCertInDB.setCertStatusChangeDate(new Date());
					mainContractCertificateHBDaoImpl.updateMainCert(mainCertInDB);
				}
			}else {
	...
	private MainContractCertWrapper populateMainCertWrapper(MainContractCertificate mainContractCertificate){
		...
		mainCertWrapper.setCertificateStatus(MainContractCertificate.CERT_CONFIRMED);
	 ...
	 */
	@SuppressWarnings("deprecation")
	private MainCertContraChargeWrapper populateMainCertContraChargeWrapper(MainContractCertificate mainContractCertificate, MainCertificateContraCharge mainCertificateContraCharge){
	...
	public String submitNegativeMainCertForApproval(String jobNumber, Integer mainCertNumber, Double certAmount, String userID) throws DatabaseOperationException{
		...
		String approvalType = MainContractCertificate.NEGATIVE_CERT_APPROVAL_ROUTE;
		String currency = "";
		try {
		...
		if (resultMsg == null || "".equals(resultMsg.trim())) {
			...
			try {
				MainContractCertificate mainCert = mainContractCertificateHBDaoImpl.obtainMainContractCert(jobNumber, mainCertNumber);
				mainCert.setCertificateStatus(MainContractCertificate.CERT_WAITING_FOR_APPROVAL);
	...
	public Boolean toCompleteMainCertApproval(	String jobNumber,
		...
		try {
			MainContractCertificate	mainCert = mainContractCertificateHBDaoImpl.obtainMainContractCert(jobNumber, Integer.valueOf(mainCertNo));
			...
			if("A".equals(approvalDecision)){
				...
				if(errorMsg==null){
					...
					mainCert.setCertificateStatus(MainContractCertificate.CERT_CONFIRMED);
					mainContractCertificateHBDaoImpl.update(mainCert);
				}
			}else{
				//Rejected Main Cert
	...
	public ByteArrayOutputStream downloadMainCertificateEnquiryReportPDF(
		...
		List<MainContractCertificate>  mainContractCertList = getMainContractCertificateList(jobNo);
		Job job = jobRepository.obtainJob(jobNo);	
	 ...
	 */
	private JasperReportHelper generateMainCertificateEnquiryReportPDF(List<MainContractCertificate> mainContractCertList, JasperReportHelper reports, List<EXPORT_OPTIONS> options, String type, String exportType) throws DatabaseOperationException, IOException, JRException{
		...
		for(MainContractCertificate temp : mainContractCertList){
			if(printIPA){
	 ...
	 */
	private JasperPrint generateMainCertificateEnquiryReportPDF_RetentionRelease(Job job, List<MainContractCertificate> mainContractCertList, String exportType) throws DatabaseOperationException, IOException, JRException{
		...
		MainContractCertificate tempCert = null;
		for(MainContractCertificate mainContractCert : mainContractCertList){
	...
	private List<MainContractCertReportRetentionReleaseWrapper> obtainRetentionReleaseWrapper(String jobNo, String cumulativeRetentionAmount){
		...
		List<RetentionRelease> retentionRelease = this.retentionReleaseScheduleHBDaoImpl.obtainRetentionReleaseScheduleByJob(jobNo);
		...
		for(RetentionRelease rr:retentionRelease){
			...
			wrapper.setReleasePercentage(decimalUtil.formatDecimal((rr.getStatus().trim().equalsIgnoreCase(RetentionRelease.STATUS_ACTUAL)?rr.getActualReleaseAmt():rr.getForecastReleaseAmt())/cumRetentionAmount*100,2));
			wrapper.setReleaseAmount(rr.getStatus().trim().equalsIgnoreCase(RetentionRelease.STATUS_ACTUAL)?decimalUtil.formatDecimal(rr.getActualReleaseAmt(),2) : decimalUtil.formatDecimal(rr.getForecastReleaseAmt(),2));
			wrapper.setStatu(rr.getStatus().trim().equalsIgnoreCase(RetentionRelease.STATUS_ACTUAL)?RR_STATUS_ACTUAL:RR_STATUS_FORECAST);
			list.add(wrapper);		
	 ...
	 */
	private JasperPrint generateMainCertificateEnquiryReportPDF_ContraCharge(Job job, String exportType) throws DatabaseOperationException, IOException, JRException{
		...
		MainContractCertificate tempMainCert = null;
		List<MainCertificateContraCharge> contras = mainCertContraChargeHBDaoImpl.obtainMainCertContraChargeList(job.getJobNumber());
		for(MainCertificateContraCharge contra : contras){
	 ...
	 */
	@SuppressWarnings("unused")
	private void generateMainCertificateEnquiryReportExcel_IPA(ExcelWorkbook doc, List<MainContractCertificate> mainContractCertList){
	 ...
	 */
	@SuppressWarnings("unused")
	private void generateMainCertificateEnquiryReportExcel_IPC(ExcelWorkbook doc, List<MainContractCertificate> mainContractCertList){
	 ...
	 */
	@SuppressWarnings("unused")
	private void generateMainCertificateEnquiryReportExcel_RetentionRelease(ExcelWorkbook doc, List<MainContractCertificate> mainContractCertList, String jobNo) throws DatabaseOperationException{
		...
		Collections.sort( mainContractCertList, new Comparator<MainContractCertificate>( ){

			@Override
			public int compare(MainContractCertificate o1,
					MainContractCertificate o2) {
					if(o1 == null || o2 == null)
						return 0;
		...
		for (RetentionRelease rr:retentionRelease){
			tempData[index] = new String[]{
					...
					rr.getStatus().trim().equalsIgnoreCase(RetentionRelease.STATUS_ACTUAL)?decimalUtil.formatDecimal(rr.getActualReleaseAmt(),2):decimalUtil.formatDecimal(rr.getForecastReleaseAmt(),2),
		...
		for (int i = 0;i<retentionRelease.size()+1;i++){
			if(i!=retentionRelease.size()){
				if(!cumulativeRetentionAmount.equals("0.00")){
					tempData[i][2] = decimalUtil.formatDecimal((retentionRelease.get(i).getStatus().trim().equalsIgnoreCase(RetentionRelease.STATUS_ACTUAL)?retentionRelease.get(i).getActualReleaseAmt():retentionRelease.get(i).getForecastReleaseAmt())/DecimalUtil.parseDouble(cumulativeRetentionAmount)*100, 2);					
	...
	private void generateMainCertificateEnquiryReportExcel_ContraCharge(ExcelWorkbook doc, String jobNo) throws DatabaseOperationException{
		...
		List<MainContractCertificate> tempMainCertList = mainContractCertificateHBDaoImpl.getMainContractCertListWithContraCharge(jobNo);		
		
		for(MainContractCertificate temp : tempMainCertList){
			List<MainCertificateContraCharge> contras = mainCertContraChargeHBDaoImpl.obtainMainCertContraChargeList(temp);
			for(MainCertificateContraCharge contra : contras){
	...
	public ExcelFile downloadMainCertificateEnquiryExcelFile(
		...
		logger.info("Start -> "+functionName);
		MainContractCertificate mainCert = getMainContractCert(jobNo, mainCertNo);
	...
	private MainContractCertificateWrapper obtainMainContractCertificateWrapper(MainContractCertificate mainCert){
	...
	public MainContractCertEnquiryResultWrapper obtainMainContractCert(
		...
		try {
			...
			List<Job> jobs = jobRepository.obtainJobsLikeByDivCoJob(wrapper.getDivisionCode(), wrapper.getCompanyNo(), wrapper.getJobNo());
			...
			PaginationWrapper<MainContractCertificate> result = new PaginationWrapper<MainContractCertificate>();
			...
			List<MainContractCertificate> list = cachedMainContractCert.subList(fromIndex, toIndex);
			List<MainContractCertificate> list2 = new ArrayList<MainContractCertificate>();
			for(MainContractCertificate cert : list){
				list2.add(cert);
			}
	...
	private Job getJob(List<Job> jobs, String jobNumber){
		for(Job job : jobs){
	...
	public ByteArrayOutputStream downloadMainCertificateEnquiryReport(
		...
		try {
			List<Job> jobs = jobRepository.obtainJobsLikeByDivCoJob(wrapper.getDivisionCode(), wrapper.getCompanyNo(), wrapper.getJobNo());
			List<MainContractCertificate> mainCertList = mainContractCertificateHBDaoImpl.obtainMainContractCertList(wrapper); 
			...
			for(MainContractCertificate temp : mainCertList){
	...
	public Double getTotalPostContraChargeAmt(MainContractCertificate mainContractCertificate) throws DatabaseOperationException {
		...
		List<MainCertificateContraCharge> contraChargeList = mainCertContraChargeHBDaoImpl.obtainMainCertContraChargeList(mainContractCertificate);
		if(contraChargeList!=null){
			for(MainCertificateContraCharge curContraCharge : contraChargeList){
