			...
			if (boolChangedLine==true) {	
				if (highlight){
					strHTMLCodingContent+=
						...
						"<td style=\"text-align: right;\">"+(amountRender((curaddendumEnquiryList.getToBeApprovedAmount()-(SCDetails.APPROVED.equals(curaddendumEnquiryList.getApproved())?curaddendumEnquiryList.getTotalAmount():0.0))))+"</td>"+
				...
				if (boolChangedLine == true) {
					if (highlight) {
						strHTMLCodingContent +=
								...
								"<td class=\"T2C2cell02\">" + (blackberryAmountRender((curaddendumEnquiryList.getToBeApprovedAmount() - (SCDetails.APPROVED.equals(curaddendumEnquiryList.getApproved()) ? curaddendumEnquiryList.getTotalAmount() : 0.0)))) + "</td>" +
								"</tr>";
					} else {
						strHTMLCodingContent +=
								"<tr class=\"first\">" +
...
//				if (scDetail instanceof SCDetailsBQ && !(scDetail instanceof SCDetailsVO)){
	...
	public MainContractCertificateHBDao getMainCertHBDao() {
		return mainCertHBDao;
	}

	public void setMainCertHBDao(MainContractCertificateHBDao mainCertHBDao) {
