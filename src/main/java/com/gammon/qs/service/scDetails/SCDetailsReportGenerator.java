...
public class SCDetailsReportGenerator {
	
	private ExcelFile excelFile;
	private List<SCDetails> scDetailsList;
	private String jobNumber;
	
	public SCDetailsReportGenerator(List<SCDetails> scPackageList, String jobNumber)
	{
...
//			excelFile.getDocument().insertRow(createContentRow(scDetails, sheetSize));
	 ...
	 */
	private void fillInContent(SCDetails scDetails, int row){
		...
		if (scDetails instanceof SCDetailsBQ){
			excelFile.getDocument().setCellValue(row, 5, ((SCDetailsBQ)scDetails).getToBeApprovedQuantity()!=null ? ((SCDetailsBQ)scDetails).getToBeApprovedQuantity().toString(): "0");
			excelFile.getDocument().setCellValue(row, 6, ((SCDetailsBQ)scDetails).getCostRate() !=null ? ((SCDetailsBQ)scDetails).getCostRate().toString() : "0");
			excelFile.getDocument().setCellValue(row, 9, ((SCDetailsBQ)scDetails).getTotalAmount()!=null ? ((SCDetailsBQ)scDetails).getTotalAmount().toString(): "0");
			excelFile.getDocument().setCellValue(row, 10, ((SCDetailsBQ)scDetails).getToBeApprovedAmount()!=null ? ((SCDetailsBQ)scDetails).getToBeApprovedAmount().toString(): "0");
		}else {
		...
		if (scDetails instanceof SCDetailsVO)
			excelFile.getDocument().setCellValue(row, 8, ((SCDetailsVO)scDetails).getToBeApprovedRate()!=null ? ((SCDetailsVO)scDetails).getToBeApprovedRate().toString(): "0");
		else
		...
		if (scDetails instanceof SCDetailsCC)
			excelFile.getDocument().setCellValue(row, 17, ((SCDetailsCC)scDetails).getContraChargeSCNo() !=null ? ((SCDetailsCC)scDetails).getContraChargeSCNo().trim(): "");
		else if (scDetails instanceof SCDetailsVO)
			excelFile.getDocument().setCellValue(row, 17, ((SCDetailsVO)scDetails).getContraChargeSCNo() !=null ? ((SCDetailsVO)scDetails).getContraChargeSCNo().trim(): "");
		else
