...
import com.gammon.qs.dao.AccountLedgerHBDao;
...
public class AccountLedgerService {
	...
	@Autowired
	private AccountLedgerHBDao			accountLedgerDao;
	@Autowired
	...
	public List<BudgetForecastWrapper> obtainAccountLedgersByJobNo(String jobNo, Integer month, Integer year) throws DatabaseOperationException {
		...
		for (BudgetForecastWrapper wrapper : wrapperList) {
			...
			List<AccountLedger> accountLedgers = accountLedgerDao.obtainAccountLedgersByBudgetForecastWrapper(wrapper);
			if (accountLedgers.size() > 0) {
				if ("1".equals(accountLedgers.get(0).getObjectCode().substring(0, 1))) {
					...
					for (AccountLedger accountLedgerToCheck : accountLedgers) {
	...
	public String uploadBudgetForecastExcel(String jobNo, String ledgerType, byte[] file) throws DatabaseOperationException {
		...
		List<AccountLedger> accountLedgerToUpdateList;
		List<AccountLedger> accountLedgerToInsertList;
		try {
			Set<String> budgetForecastHashMap = new HashSet<String>();
			HashMap<Long, AccountLedger> accountLedgerHashMap = new HashMap<Long, AccountLedger>();

			accountLedgerToUpdateList = new ArrayList<AccountLedger>();
			accountLedgerToInsertList = new ArrayList<AccountLedger>();
			// Open Excel File
			...
			List<AccountLedger> existingRecords = accountLedgerDao.obtainAccountLedgersByJobNo(jobNo, ledgerType);
			for (AccountLedger existingRecord : existingRecords) {
			...
			for (BudgetForecastExcelWrapper budgetForecastExcelWrapper : budgetForecastExcelWrapperList) {
				...
				AccountLedger existedAccountLedger = accountLedgerDao.obtainAccountLedgerByAccountLedger(accountLedger);
			...
			for (AccountLedger accountLedgerToUpdateTemp : accountLedgerToUpdateList) {
				accountLedgerHashMap.remove(accountLedgerToUpdateTemp.getId());
				accountLedgerDao.update(accountLedgerToUpdateTemp);
			}

			ArrayList<AccountLedger> accountLedgerToUpdateArrayList = new ArrayList<AccountLedger>(accountLedgerHashMap.values());

			// Update records: reset the rest to zero
			for (AccountLedger accountLedgerToUpdate : accountLedgerToUpdateArrayList) {
				if ("FC".equals(ledgerType)) {
			...
			for (AccountLedger accountLedgerToInsert : accountLedgerToInsertList) {
	...
	private AccountLedger setContractTotal(AccountLedger accountLedger, int month, double contractTotal) {
		switch (month) {
		case 1:
