...
public class RetentionReleaseScheduleService implements Serializable {
	...
	@Autowired
	private transient RetentionReleaseScheduleHBDao retentionReleaseScheduleHBDao;
	@Autowired
	private transient JobHBDao jobHBDao;
	@Autowired
	private transient JobWSDao jobWSDao;
	@Autowired
	private transient MainContractCertificateHBDao mainContractCertificateDao;
	@Autowired
	private transient MainContractCertificateWSDao mainContractCertificateWSDao;
	@Autowired
	...
	public RetentionReleaseScheduleHBDao getRetentionReleaseScheduleHBDao() {
		return retentionReleaseScheduleHBDao;
	}
	public void setRetentionReleaseScheduleHBDao(
			RetentionReleaseScheduleHBDao retentionReleaseScheduleHBDao) {
	...
	public String obtainCumulativeRetentionReleaseByJob(MainContractCertificate mainCert) throws DatabaseOperationException{
		...
		List<RetentionRelease> rrList = retentionReleaseScheduleHBDao.obtainRetentionReleaseScheduleByJob(mainCert.getJobNo());
		...
		if (rrList!=null && rrList.size()>0)
			for (RetentionRelease rr:rrList){
				if (RetentionRelease.STATUS_ACTUAL.equals(rr.getStatus())){
	...
	public List<RetentionRelease> calculateRetentionReleaseScheduleByJob(MainContractCertificate mainCert) throws DatabaseOperationException{
		...
		try {
			...
			if (securityList.contains("WRITE")){
				List<MainContractCertificate> mainCertList = mainContractCertificateRepository.getMainContractCertificateList(jobNumber);
				ArrayList<RetentionRelease> newRRSList = new ArrayList<RetentionRelease>();
				MainContractCertificate prevMainCert = null;
				...
				if (mainCertList!=null && mainCertList.size()>0)
					for (int i=0;i<mainCertList.size();i++){//Calculate all certs
						...
						RetentionRelease dbRR = retentionReleaseScheduleHBDao.obtainActualRetentionReleaseByMainCertNo(jobNumber, mainCertList.get(i).getCertificateNumber());
							if(dbRR==null){
								RetentionRelease newRR = new RetentionRelease();
								...
								newRR.setStatus(RetentionRelease.STATUS_ACTUAL);
	 ...
	 * modified on 18 Dec, 2013
	 * **/
	private void updateActualRetentionRelease(String jobNumber, MainContractCertificate mainCert, MainContractCertificate prevMainCert, List<RetentionRelease> newRRSList) throws DatabaseOperationException {
		...
		RetentionRelease rr = retentionReleaseScheduleHBDao.obtainActualRetentionReleaseByMainCertNo(jobNumber, mainCert.getCertificateNumber());
		...
		if(rr!=null && (mainCert.getCertificateNumber().equals(rr.getMainCertNo()))){
			...
			if (rrDiff!=0.00 && dbRRDiff!=0.00){
				...
				rr.setStatus(RetentionRelease.STATUS_ACTUAL);
		...
		for(RetentionRelease rrToInsert: newRRSList){
	...
	private double calRetentionReleaseAmount(MainContractCertificate mainCert){
		if (mainCert==null)
			return 0.0;
		return 
	...
	public Boolean updateForecastRetentionRelease(String jobNo, List<RetentionRelease> modifiedList) throws DatabaseOperationException{
		logger.info("STARTED - updateForecastRetentionRelease");
		Boolean updated = false;
		List<RetentionRelease> rrToUpdateList = new ArrayList<RetentionRelease>();
		List<RetentionRelease> rrToInsertList = new ArrayList<RetentionRelease>();
		
			for(RetentionRelease rr: modifiedList)
				rrToInsertList.add(rr);
			
			List<RetentionRelease> dbList = retentionReleaseScheduleHBDao.obtainRetentionReleaseScheduleByJob(jobNo);

			for(RetentionRelease dbRR: dbList){
			...
			for(RetentionRelease rrToUpdate: rrToUpdateList){//update records
				logger.info("Update retention release id:"+rrToUpdate.getId());
				retentionReleaseScheduleHBDao.merge(rrToUpdate);
				updated=true;
			}
			
			for (RetentionRelease rrToInsert: rrToInsertList){//insert new records
	...
	public ArrayList<RetentionReleaseScheduleEnquiryWindowsWrapper> obtainRetentionReleaseSchedule(
		...
		List<Job> jobList = jobHBDao.obtainJobsByDivCoJob(division,company,jobNo);
		...
		List<RetentionRelease> rrList = new ArrayList<RetentionRelease>();
		for (Job job:jobList)
		...
		for (RetentionRelease rr:rrList){
			if (aJob==null || !aJob.getJobNumber().equals(rr.getJobNumber())){
				aJob = jobHBDao.obtainJob(rr.getJobNumber());
				try {
			...
			try {
				Job job = jobHBDao.obtainJob(rr.getJobNumber());
			...
			try {
				MainContractCertificate latestPostedCert = mainContractCertificateDao.searchLatestPostedCert(rr.getJobNumber());
			...
			if (RetentionRelease.STATUS_ACTUAL.equals(rr.getStatus()))
				try {
					...
					MainContractCertificate thisMainCert = mainContractCertificateDao.obtainMainContractCert(rr.getJobNumber(), rr.getMainCertNo());
					if (thisMainCert.getCertificateStatus()!=null && Integer.parseInt(thisMainCert.getCertificateStatus().trim())<300 )
						rrwrapper.setStatus(RetentionRelease.STATUS_NOT_YET_RECEIVE);
				} catch (Exception e) {
					logger.info(e.getLocalizedMessage());
				}
			else
	...
	public ArrayList<RetentionReleaseScheduleEnquiryWindowsWrapper> obtainRetentionReleaseSchedule(
		...
		List<Job> jobInJobSecurityList = jobHBDao.obtainJobsByDivCoJobList(division,company,jobNoInJobSecurityList);
		List<String> jobNoList = new ArrayList<String>();
		if (jobInJobSecurityList==null || jobInJobSecurityList.size()<1)
			return new ArrayList<RetentionReleaseScheduleEnquiryWindowsWrapper>();
		List<RetentionRelease> rrList = new ArrayList<RetentionRelease>();
		for (Job job:jobInJobSecurityList){
			jobNoList.add(job.getJobNumber().trim());
		}
		for (int i = 0; i*MAX_CRITERIA_LIST<jobNoList.size();i++)
			if ((i+1)*MAX_CRITERIA_LIST>jobNoList.size())
