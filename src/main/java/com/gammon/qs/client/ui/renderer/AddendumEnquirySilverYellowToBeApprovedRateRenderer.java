...
public class AddendumEnquirySilverYellowToBeApprovedRateRenderer extends CustomizableRenderer  implements Renderer{

	public AddendumEnquirySilverYellowToBeApprovedRateRenderer(User user) {
	...
	public String render(Object value, CellMetadata cellMetadata, Record record,
		...
		try{
			...
			if (store.getAt(rowIndex).getAsString("sourceType").equals("BQ")){
				return "<table width=\"100%\" height=\"100%\"><tr><td style=\"background-color:#D3D3D3;\"><font color=#D3D3D3>"+ format.format(raw.doubleValue())+"</font></td></tr></table>";                 	
			} else if (store.getAt(rowIndex).getAsString("sourceType").equals("A") && !store.getAt(rowIndex).getAsString("lineType").trim().equals("OA")) {
				if (!store.getAt(rowIndex).getAsString("approved").equals("A")){
					if (store.getAt(rowIndex).getAsDouble("scRate")!=(store.getAt(rowIndex).getAsDouble("toBeApprovedRate"))) {
						...
						if (!store.getAt(rowIndex).getAsString("approved").equals("S")) {
							return "<table width=\"100%\" height=\"100%\"><tr><td style=\"background-color:#FFFF00;\"><font color=#000000>"+ format.format(raw.doubleValue())+"</font></td></tr></table>";                 
						} else {
...
public class AddendumEnquirySilverYellowToBeApprovedRateRenderer extends CustomizableRenderer implements Renderer{
	...
	public String render(Object value, CellMetadata cellMetadata, Record record,
		...
		try{
			...
			//return format.format(raw.doubleValue());
			if (store.getAt(rowIndex).getAsString("sourceType").equals("BQ")){
				return "<table width=\"100%\" height=\"100%\"><tr align = RIGHT><td style=\"background-color:#D3D3D3;\"><font color=#D3D3D3>"+ format.format(raw.doubleValue())+"</font></td></tr></table>";                 	
			} else if (store.getAt(rowIndex).getAsString("sourceType").equals("A") && !store.getAt(rowIndex).getAsString("lineType").trim().equals("OA")) {
