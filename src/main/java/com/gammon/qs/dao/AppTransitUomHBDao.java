...
public class AppTransitUomHBDao extends BaseHibernateDao<AppTransitUom> {
	Logger logger = Logger.getLogger(AppTransitUomHBDao.class.getName());
	
	public AppTransitUomHBDao() {
		super(AppTransitUom.class);
	}
	
	public String saveAll(List<AppTransitUom> codeMatches) throws Exception{
		...
		try{
			...
			e.printStackTrace();
			AppTransitUom codeMatch = codeMatches.get(i);
