...
//		}		
	...
	public void generateAccountBalanceExcel(@RequestParam(required=true,value="jobNumber") String jobNumber,
									 ...
									 @RequestParam(required=true,value="totalFlag") String totalFlag,
									 ...
									 @RequestParam(required=true,value="year") String year,
									 @RequestParam(required=true,value="period") String period,
									 HttpServletRequest request, HttpServletResponse response ){		
		
		logger.info("generateAccountBalanceExcel");
		...
		try {
			...
			ExcelFile excelFile = jobCostRepository.downloadAccountBalanceExcelFile(jobNumber, subLedger, subLedgerType, totalFlag, postFlag, fromDate, thruDate, year, period);
			...
			if(excelFile != null){
				...
				logger.info("Download Controller : Account Payable Excel Download - Excel Fiel Error (File is Null)");
				showReportError(response);
			}
	...
	/*
	 * add by paulyiu 20150722
	 */
	@RequestMapping(value="/gammonqs/accountBalanceReportExport.rpt",method = RequestMethod.GET)
	public void generateAccountBlanceReport(@RequestParam(required=true,value="jobNumber") String jobNumber,
											@RequestParam(required=false,value="subLedger") String subLedger,
											@RequestParam(required=false,value="subLedgerType") String subLedgerType,
											@RequestParam(required=false,value="totalFlag") String totalFlag,
											@RequestParam(required=false,value="postFlag") String postFlag,
											@RequestParam(required=false,value="fromDate") String fromDateString,
											@RequestParam(required=false,value="thruDate") String thruDateString,
											...
											HttpServletRequest request, HttpServletResponse response) {
		logger.info("JobCostReportDownload");
		try {
			String jasperReportName = "JobCostReport";
			Date fromDate = fromDateString.equals("") ? null : DateUtil.parseDate(fromDateString, "dd/MM/yyyy");
			Date thruDate = thruDateString.equals("") ? null : DateUtil.parseDate(thruDateString, "dd/MM/yyyy");
			ByteArrayOutputStream outputStream = jobCostRepository.downloadAccountBalanceReportFile(jobNumber, subLedger, subLedgerType, totalFlag, postFlag, fromDate, thruDate, year, period, jasperReportName, fileType);
			if (outputStream != null) {
				response.setContentType(RESPONSE_CONTENT_TYPE_APPLICATION_OCTENT_STREAM);
				response.setHeader(RESPONSE_HEADER_NAME_CONTENT_DISPOSITION, "attachment; filename=\"" + "J" + jobNumber + " Job Cost Report " + DateUtil.formatDate(new Date(), "yyyy-MM-dd HHmmss") + "." + fileType + "\"");
				response.getOutputStream().write(outputStream.toByteArray());
				response.getOutputStream().flush();
			} else {
				logger.info("No file is generated.");
