...
public class FileHelper {
	...
	public static String convertFileToString(String htmlFilePath) {
		...
		try {
			...
			Path path = null;
			if(htmlFilePath.indexOf("classpath:") > -1){
				String filePath = htmlFilePath.substring(htmlFilePath.indexOf("/"));
				path = Paths.get(FileHelper.class.getResource(filePath).toURI());
			} else if(htmlFilePath.indexOf("file:") > -1){
				String filePath = htmlFilePath.substring(htmlFilePath.indexOf("/")+1);
