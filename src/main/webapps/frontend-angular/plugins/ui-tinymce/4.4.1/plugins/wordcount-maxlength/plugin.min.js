...
//			charLimit = editor.getElement().dataset.maxLength;
		//maximum character length set by tinyOptions.maxLength
		
		maxLength = editor.editorManager.settings.maxLength;
		charLimit = maxLength ? maxLength : 3000;

		if (charLimit != 'undefined' && self.getCharCount() > charLimit) {
			wc.text(['You\'ve reached the max. limit of text length!']);
			// mce-danger class added to the wordcount status in the statusbar
			$('#'+ wc[0]._id).css('color', 'orange');
			if (wc.length > 1) {
				$('#'+ wc[1]._id).css('color', 'orange');
			}
			tinyMceCharLimitReached = true;
		} else {
