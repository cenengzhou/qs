mainApp.controller('JobPersonnel2Ctrl', [
		'$scope', '$rootScope', '$q', '$log', '$timeout', '$cookies', '$filter', '$mdDialog', 'rootscopeService', 'jobService', 'modalService', 'personnelService',
		function($scope, $rootScope, $q, $log, $timeout, $cookies, $filter, $mdDialog, rootscopeService, jobService, modalService, personnelService) {
...
//					if(person.personnelMap.requiredApproval=='Y') self.requiredApproval--;
			...
			function showApproval(id, ev) {
				...
				personnelService.returnDecisionMap(self.job.noReference)
				.then(function(data){
					$scope.returnDecisionMap = data;
				    $mdDialog.show({
		    	        parent:parentEl,
		    	        clickOutsideToClose: true,
		    	        templateUrl: 'show-approval-route.html',
	    	            locals: {
	    	                decisionMap: $scope.returnDecisionMap
	    	            },
		    	        targetEvent: ev,
			...
			function approvalRouteCtrl ($scope, $mdDialog, decisionMap) {
